...
{
  currMin = max(hookMeasurements$hook)
...
shinyServer(function(input, output, session) {
  hookMeasurements <- reactive({
    output$numWorms<-reactive({""})
    inFile <- input$file1
    if (is.null(inFile))
      return(NULL)
  ...
  meristogramData <- reactive({
    if (is.null(hookMeasurements()))
      return(NULL)
    ...
    if(input$movAvgIntervalSlider>minMovAvgSeg)
      movAvgSeg <- input$movAvgIntervalSlider
    else
      movAvgSeg <- minMovAvgSeg
    
    updateSliderInput(session, "movAvgIntervalSlider", value = movAvgSeg, min = minMovAvgSeg, max = 30)
    
    createMeristogram(hookMeasurements(), movAvgSeg, lerp = input$lerp)
    
  })
  
  output$meristoPlot <- renderPlot({   
    if (is.null(hookMeasurements()))
      return(NULL)
    if (is.null(meristogramData()))
    
    validInput <- validate(
      need(identical(names(hookMeasurements()), c("specimen", "hook", "length", "base")), "File of hook measurements must be comma-separated with four columns: specimen, hook, length and base")
    )
    
    numWorms <- length(unique(hookMeasurements()$specimen))
    
    par(mar = c(5.1, 4.1, 0, 1))
    plotMeristogram(meristogramData())
    
    output$numWorms<-reactive({paste("Number of worms:", numWorms, sep=" ")})
  })
  
  output$downloadData <- downloadHandler(
    filename = function() { 
      if(input$lerp==TRUE){
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval_lerp.csv", sep='')
      }else{
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval.csv", sep='')
      }
    },
    content = function(file) {
      write.csv(meristogramData(), file, row.names=F, quote=F)
    }
  )
  ...
  output$downloadPDF <- downloadHandler(
    filename = function() { 
      if(input$lerp==TRUE){
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval_lerp.pdf", sep='')
      }else{
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval.pdf", sep='')
      }
    },
    content = function(file) {
      pdf(file, width=7, height=4, title="Meristogram")
      par(mar=c(4, 4, 1, 1) + 0.1)
      plotMeristogram(meristogramData(), cex=0.7)
      dev.off()
    }
  )
  ...
  output$downloadPNG <- downloadHandler(
    filename = function() { 
      if(input$lerp==TRUE){
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval_lerp.png", sep='')
      }else{
        paste("meristogram_", input$movAvgIntervalSlider, "_percent_mov_avg_interval.png", sep='')
      }
    },
    content = function(file) {
      png(file, res=1000, units="mm", width=180, height=120)
      par(mar=c(4, 4, 1, 1) + 0.1)
      plotMeristogram(meristogramData(), cex=0.8)
      dev.off()
    }
  )
